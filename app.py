...
def preview_file_full(file, required_columns):
    try:
        df = pd.read_csv(file)
        df.columns = df.columns.str.strip().str.lower()  # forgiving column names
        ...
        for kid, prefs in kids_prefs.items():
            ...
            if rank < len(prefs):
                ...
                available_slots = [
                    ...
                    if k[0].lower() == prefs[rank].lower() and program_slots[k] > 0 and str(k[2]) not in occupied_slots
                ]
                if available_slots:
    ...
    if df_programs is not None and df_kids is not None:
        ...
        for kid, progs in assignments.items():
            for p in progs:
                ...
                rank = next((prefs.index(prog_name)+1 for i in range(len(prefs)) if prog_name.lower()==prefs[i].lower()), len(prefs)+1)
                table_rows.append({
                    'Kid': kid,
                    'Program': prog_name,
                    'Day': day,
